class VendingMachine:
    ...
    def vend(self):
        ...
        self.need = self.product_price - self.balance
        if self.need > 0:
            return 'You must add ${} more funds.'.format(self.need)
        elif self.need < 0:
            self.balance =0
            self.product_num -= 1
            return  'Here is your {} and ${} change.'.format(self.product_name,-self.need)
        else:
            self.balance =0
            self.product_num -= 1
...
def store_digits(n):
    ...
    def helper(n, acc):
        ...
        return helper(remaining_digits, Link(last_digit, acc))
...
def path_yielder(t, value):
    ...
    "*** YOUR CODE HERE ***"
    def inner_yield(t, value, path):
        path = path + [t.label]
        if t.label == value:
            yield path
        for b in t.branches:
            yield from inner_yield(b, value, path)

    yield from inner_yield(t, value, [])
