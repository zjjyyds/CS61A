...
def balanced(m):
    ...
    def is_balanced_arm(a):
        """检查给定臂是否是平衡的。"""
        return is_planet(end(a)) or balanced(end(a))

    if is_planet(m):
        return True
    else:
        left_arm, right_arm = left(m), right(m)
        left_torque, right_torque = torque(left_arm), torque(right_arm)

        if left_torque != right_torque:
            return False

        return is_balanced_arm(left_arm) and is_balanced_arm(right_arm)
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        total_weight_m = total_weight(m)
        left_branch = totals_tree(end(left(m)))
        right_branch = totals_tree(end(right(m)))
        return tree(total_weight_m, [left_branch, right_branch])
...
def multiple_references_explanation():
    return """确实，par2 的实现中使用了多个引用，避免了同一区间的多次引用可能会导致误差边界变得松散，
    从而影响最终结果的准确性问题，因此在某些情况下可以产生更精确的结果。"""
