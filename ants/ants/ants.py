...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        Asks the insect to remove itself from the current place. This method exists so
            it can be enhanced in subclasses.
        """
        if insect.name =="Queen" and not insect.imposter:
            return None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        bees = None
        transition = 0
        while not bees and place != beehive:
    ...
    def action(self, gamestate):
        ...
        if self.imposter:
            ...
            while curr_place.exit:
                ...
                if ant and ant not in self.ants_doubled:
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 3
        for bee in self.place.bees[:]:
            bee.reduce_armor(TankAnt.damage)
        # END Problem Optional 3
############
# Statuses #
############
    ...
    def insects_in_front(self, beehive):
        ...
        while cur_place is not beehive:
            if cur_place.ant and cur_place.ant is not self:
                all_insects[cur_place.ant] = distance
            for b in cur_place.bees[:]:
                all_insects[b] = distance
            distance += 1
