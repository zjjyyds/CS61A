def couple(s, t):
    ...
    couple_list = [[s[i],t[i]] for i in range(len(s))]
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    tem = make_city('',lat,lon)
    if distance(city_a,tem)<=distance(city_b,tem):
...
def riffle(deck):
    ...
    """
    mid = len(deck) // 2
    return [deck[i // 2] if i % 2 == 0 else deck[mid + i // 2] for i in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    if t1 is None:
        ...
        # 扩展分支列表至相同长度
        branches1 = branches(t1) + [None] * (len2 - len1)
        branches2 = branches(t2) + [None] * (len1 - len2)
        return tree(label(t1) + label(t2), [add_trees(b1, b2) for b1, b2 in zip(branches1, branches2)])
...
def construct_sent(word, table):
    ...
    while word not in ['.', '!', '?']:
        ...
        word = random.choice(successors) if successors else '.'
