...
def roll_dice(num_rolls: object, dice: object = six_sided) -> object:
    ...
    totol_sum = 0
    is_one=0
    """为什么不能出现1的时候直接return，因为可能会在一次运行的时候多次调用同一个dice，这个时候如果循环到一半return
    在下次调用dice时就会影响随机数的生成
    可以标记是否出现了1
    """
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    strategy1:  The strategy function for Play2
    er 1, who plays second.
...
def make_averaged(original_function, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def get_average(*args: object) -> object:
        result =0
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_dice =0
    for i in range(1,11):
        average_total=make_averaged(roll_dice)
        k = average_total(i, dice)
        if k>max_total:
            max_total=k
            max_dice=i
        elif k==max_total:
...
def final_strategy(score, opponent_score):
    ...
    1.根据当前分数动态设置cutoff,分数越高cutoff越低
    2.首先判断额外回合策略,执行则返回0
    其次判断免费培根策略,满足则返回0
    如果分数低于对手,投最大骰子数6
    如果分数高于对手,投较少骰子数3
